...
class KeyPatchGanModel():
    ...
    def forward(self):
        ...
        self.parts_enc = {
            ...
            'e4': self.part1_enc_out['e4'] + self.part2_enc_out['e4'] + self.part3_enc_out['e4'],
            'e5': self.part1_enc_out['e5'] + self.part2_enc_out['e5'] + self.part3_enc_out['e5']
        }
        ...
        self.gen_mask, gen_mask_output = self.net_mask_generator(self.parts_enc['embed'],
                                                ...
                                                self.parts_enc['e4'], self.parts_enc['e5'])

        ''' Generating Full image'''
        self.image_gen = self.net_generator(self.parts_enc['embed'], self.input_z,
                                              gen_mask_output['m0'], gen_mask_output['m1'],
                                              gen_mask_output['m2'], gen_mask_output['m3'],
                                              gen_mask_output['m4'], gen_mask_output['m5'])
    ...
    def backward_G(self):
        ...
        self.g_loss = self.weight_g_loss * self.g_loss_l1_appr + \
                      self.weight_g_loss * self.g_loss_l1_mask + \
                      1.0 * self.g_loss_gan
...
class PartEncoder(nn.Module):
    def __init__(self, opts):
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(self.opts.df_dim * 8, self.opts.df_dim * 8,
                      kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(self.opts.df_dim * 8),
            nn.LeakyReLU(0.2),
        )
        ...
        conv5_spatial_dim = np.int(opts.output_size / np.power(2, self.num_conv_layers))
        self.fc6 = nn.Sequential(
            nn.Conv2d(self.opts.df_dim*8, self.opts.part_embed_dim,
                       kernel_size=conv5_spatial_dim, stride=1, padding=0, bias=False),
    ...
    def forward(self, x):
        ...
        self.e4 = self.conv4(self.e3)
        self.e5 = self.conv5(self.e4)
        self.embed = self.fc6(self.e5)
    ...
    def __init__(self, opts):
        ...
        self.convT0 = nn.Sequential(
            nn.ConvTranspose2d(df_dims_in[0], df_dims_out[0],
                               kernel_size=conv3_spatial_dim, stride=1, padding=0, bias=False),
        ...
        self.convT5 = nn.Sequential(
            nn.ConvTranspose2d(df_dims_in[5], df_dims_out[5],
                               kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(opts.df_dim),
            nn.ReLU(),
        )
        self.convT6 = nn.Sequential(
            nn.ConvTranspose2d(df_dims_in[6], df_dims_out[6],
                               kernel_size=4, stride=2, padding=1, bias=False),
            nn.Sigmoid()
        )

    def forward(self, embed, e0, e1, e2, e3, e4, e5):
        self.m0 = self.convT0(embed)
        self.m0 = torch.cat([self.m0, e5], 1)
        self.m1 = self.convT1(self.m0)
        self.m1 = torch.cat([self.m1, e4], 1)
        self.m2 = self.convT2(self.m1)
        self.m2 = torch.cat([self.m2, e3], 1)
        self.m3 = self.convT3(self.m2)
        self.m3 = torch.cat([self.m3, e2], 1)
        self.m4 = self.convT4(self.m3)
        self.m4 = torch.cat([self.m4, e1], 1)
        self.m5 = self.convT5(self.m4)
        self.m5 = torch.cat([self.m5, e0], 1)
        self.mask = self.convT6(self.m5)
...
class Discriminator(nn.Module):
    def __init__(self, opts):
        ...
        self.conv0 = nn.Sequential(
            nn.Conv2d(self.opts.c_dim, self.opts.df_dim * 2,
                      kernel_size=4, stride=2, padding=1, bias=False),
            nn.LeakyReLU(0.2)
        )
        self.conv1 = nn.Sequential(
            nn.Conv2d(self.opts.df_dim * 2, self.opts.df_dim * 2 * 2,
                      kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(self.opts.df_dim * 2 * 2),
            nn.LeakyReLU(0.2),
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(self.opts.df_dim * 2 * 2, self.opts.df_dim * 4 * 2,
                      kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(self.opts.df_dim * 4 * 2),
            nn.LeakyReLU(0.2),
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(self.opts.df_dim * 4 * 2, self.opts.df_dim * 8 * 2,
                      kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(self.opts.df_dim * 8 * 2),
            nn.LeakyReLU(0.2),
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(self.opts.df_dim * 8 * 2, self.opts.df_dim * 8 * 2,
                      kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(self.opts.df_dim * 8 * 2),
            nn.LeakyReLU(0.2),
        )
        ...
        conv5_spatial_dim = np.int(opts.output_size / np.power(2, self.num_conv_layers))
        self.fc6 = nn.Sequential(
            nn.Conv2d(self.opts.df_dim * 8 * 2, 1,
                      kernel_size=conv5_spatial_dim, stride=1, padding=0, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, img):
        ...
        self.d4 = self.conv4(self.d3)
        self.d5 = self.conv5(self.d4)
        self.disc = self.fc6(self.d5)
        return self.disc
...
class ImageGenerator(nn.Module):
    def __init__(self, opts):
        ...
        self.convT0 = nn.Sequential(
            nn.ConvTranspose2d(df_dims_in[0], df_dims_out[0],
                               kernel_size=conv3_spatial_dim, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(opts.df_dim * 8 * 2),
            nn.ReLU()
        )
        self.convT1 = nn.Sequential(
            nn.ConvTranspose2d(df_dims_in[1], df_dims_out[1],
        ...
        self.convT3 = nn.Sequential(
            nn.ConvTranspose2d(df_dims_in[3], df_dims_out[3],
                               kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(opts.df_dim * 4 * 2),
            nn.ReLU(),
        )
        self.convT4 = nn.Sequential(
            nn.ConvTranspose2d(df_dims_in[4], df_dims_out[4],
                               kernel_size=4, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(opts.df_dim * 2 * 2),
            nn.ReLU(),
        )
        self.convT5 = nn.Sequential(
            nn.ConvTranspose2d(df_dims_in[5], df_dims_out[5],
    ...
    def forward(self, embed, z, m0, m1, m2, m3, m4, m5):
        ...
        self.g4 = self.convT4(self.g3)
        self.g4 = torch.cat([self.g4, m4], 1)
        self.g5 = self.convT5(self.g4)
        self.g5 = torch.cat([self.g5, m5], 1)
        self.gen_image = self.convT6(self.g5)
